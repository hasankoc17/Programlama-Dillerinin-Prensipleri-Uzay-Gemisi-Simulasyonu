...
public class Simulasyon {
    ...
    private boolean kesinImha(UzayAraci a){
        if (a.getDurum()==UzayAraci.Durum.IMHA) return true;  // zaten imha
        if (a.getDurum()==UzayAraci.Durum.VARDI) return false; // hedefine ulaştı → tarihle göster
        if (a.getYolcular().isEmpty()) return true;            // bekliyor/yolda ve yolcu yok → imha
        long bek = (a.getDurum()==UzayAraci.Durum.BEKLIYOR)
                ? Math.max(0, a.getCikisGezegeni().getZaman()
    ...
    private static void temizle() {
        ...
        try {
            if (os.contains("win")) {
                // Windows: önce ANSI dener, başarısızsa cmd /c cls
                System.out.print("\033[H\033[2J");
                System.out.flush();
                // CMD hâlâ doluysa ProcessBuilder ile cls
        ...
        } catch (Exception ignored) { /* Sessizce geç */ }
    }
